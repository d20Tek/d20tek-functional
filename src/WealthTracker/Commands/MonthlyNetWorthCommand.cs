...
internal static class MonthlyNetWorthCommand
{
    ...
    [
        ...
        .. dateRange.Select(date => MoneyPresenter.Render(entry.GetLatestValueFor(date))),
        MoneyPresenter.Render(entry.GetLatestValue())
    ];
    ...
    [
        ...
        [
            ...
            .. dateRange.Select(d => MoneyPresenter.Render(entries.Sum(x => x.GetLatestValueFor(d)))),
            MoneyPresenter.Render(entries.Sum(x => x.GetLatestValue())),
        ]
    ];
